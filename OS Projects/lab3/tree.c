...
struct Node* newNode(int key, char* string)
{
...
// The string should contain its integer key at the beginning
struct Node* insert(struct Node* node, char* string)
{
    int key = atoi(string);
    /* 1.  Perform the normal BST insertion */
    if (node == NULL){
        struct Node * t = newNode(key, string);
        printf(" inserted %s", t->string);
